...
client.on('message', (receivedMessage) => {
    ...
    if (receivedMessage.content.toLowerCase() == '-help') return helpCommand(receivedMessage);

    if (receivedMessage.content.toLowerCase() == '-rules') return rulesCommand(receivedMessage);

    if (player == undefined) {
        if (receivedMessage.content.toLowerCase() == '-play') {
            player = receivedMessage.author.id;
            newGameCommand(receivedMessage);
        }
    } else if (receivedMessage.author.id == player) {

        if (receivedMessage.content.toLowerCase() == '-stop') return endGameCommand(receivedMessage);
...
function rulesCommand(receivedMessage) {
    ...
    .setColor('#000000')
    .setTitle('arduino-GameConsole: RULES')
    .setAuthor('arduino-GameConsole', 'https://i.imgur.com/jOz6aGJ.png')
...
function helpCommand(receivedMessage) {
    ...
    .setColor('#000000')
    .setTitle('arduino-GameConsole')
    .setAuthor('arduino-GameConsole', 'https://i.imgur.com/jOz6aGJ.png')
    .setDescription('Hello! I am the \'arduino-GameConsole\' Bot\n\n' +
                    'To play the game, type `-play`\n' +
                    'To stop the game in between, type `-stop`\n' +
                    'To see the rules, type `-rules`\n' +
                    '\nAs always, have fun!');
